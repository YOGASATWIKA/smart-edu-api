...
import (
	...
	summary2 "smart-edu-api/service/generator/materi/genai/summary"
)

type Fetch struct {
	sum *summary2.Summary
}

func NewFetch(sum *summary2.Summary) *Fetch {
	return &Fetch{
		sum: sum,
	}
}
	...
	for _, part := range ebook.Parts {
		for _, chapter := range part.Chapters {
			...
			for {
				...
				out, err := f.sum.Generate(ctx, summary2.Prompt{
