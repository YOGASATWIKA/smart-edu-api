...
import (
	...
	background_urgency2 "smart-edu-api/service/generator/materi/genai/background_urgency"
)

type Fetch struct {
	bg *background_urgency2.Intro
}

func NewFetch(bu *background_urgency2.Intro) *Fetch {
	return &Fetch{
		bg: bu,
	}
}
...
func (f *Fetch) Fetch(ctx context.Context, ebook *entity.Ebook) {
	for _, part := range ebook.Parts {
		...
		for {
			...
			out, err := f.bg.Generate(ctx, background_urgency2.Prompt{
