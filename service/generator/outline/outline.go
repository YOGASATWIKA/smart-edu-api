...
func (o *Outliner) Generate(model string, ctx context.Context, params Params) (entity.Outline, error) {
	...
	conversationHistory := []llms.MessageContent{
		llms.TextParts(llms.ChatMessageTypeSystem, existing.Promt.SystemPrompt),
	}
	...
	for _, stepPrompt := range promptSteps {
		...
		res, err := o.model.GenerateContent(ctx, conversationHistory, llms.WithJSONMode(), llms.WithMaxTokens(6000)) // Max token dinaikkan
		if err != nil {
